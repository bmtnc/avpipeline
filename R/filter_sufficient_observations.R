...
filter_sufficient_observations <- function(data, group_col, min_obs) {
  if (!is.character(group_col) || length(group_col) != 1) {
    stop(paste0(
      "Argument 'group_col' must be single character string, received: ",
      class(group_col)[1],
      " of length ",
      length(group_col)
    ))
  ...
  if (!is.numeric(min_obs) || length(min_obs) != 1) {
    stop(paste0(
      "Argument 'min_obs' must be single numeric value, received: ",
      class(min_obs)[1],
      " of length ",
      length(min_obs)
    ))
  ...
  if (is.na(min_obs) || min_obs < 1) {
    stop(paste0(
      "Argument 'min_obs' must be positive integer, received: ",
      min_obs
    ))
  }
