...
calculate_baseline_stats <- function(values, indices) {
  ...
  if (!is.numeric(indices) || length(indices) == 0) {
    stop(paste0("Argument 'indices' must be non-empty integer vector, received: ", 
                class(indices)[1], " of length ", length(indices)))
  }
  
  if (any(indices < 1) || any(indices > length(values))) {
    stop(paste0("Argument 'indices' contains out-of-bounds values. Valid range: 1 to ", 
                length(values)))
  }
