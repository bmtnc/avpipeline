...
#' @keywords internal
summarize_artifact_construction <- function(original_data, final_data, removed_detail = NULL) {
  if (!is.data.frame(original_data)) {
    stop(paste0("summarize_artifact_construction(): [original_data] must be a data.frame, not ", class(original_data)[1]))
  }
  
  if (!is.data.frame(final_data)) {
    stop(paste0("summarize_artifact_construction(): [final_data] must be a data.frame, not ", class(final_data)[1]))
  }
  
  if (!is.null(removed_detail) && !is.data.frame(removed_detail)) {
    stop(paste0("summarize_artifact_construction(): [removed_detail] must be a data.frame or NULL, not ", class(removed_detail)[1]))
  }
  ...
  if (removed_obs > 0) {
    ...
    if (!is.null(removed_detail) && nrow(removed_detail) > 0) {
      ...
      for (i in seq_len(nrow(removed_detail))) {
        ...
        message(paste0("  ", ticker, ": ", count, " observations removed (", as.character(earliest), " to ", as.character(latest), ")"))
      }
    }
  }
  ...
  message("\u2713 All observations validated using fiscal-pattern quarterly validation")
  ...
  if (nrow(gaps_to_analyze) > 0) {
    ...
    message(paste0("- Range: ", gap_stats$min_days, " to ", gap_stats$max_days, " days"))
    message(paste0("- Average gap: ", gap_stats$avg_days, " days"))
    message(paste0("- Median gap: ", gap_stats$median_days, " days"))
    message(paste0("- Gaps in 80-100 day range: ", gap_stats$gaps_80_to_100_pct, "%"))
  }
  
  has_complete_financials_col <- "has_complete_financials" %in% names(final_data)
  ...
  message(paste0("- Date range: ", as.character(min(final_data$fiscalDateEnding)), " to ", as.character(max(final_data$fiscalDateEnding))))
  
  if (has_complete_financials_col) {
    message(paste0("- Complete financial records: ", sum(final_data$has_complete_financials)))
  }
  
  if (has_earnings_metadata_col) {
    message(paste0("- Records with earnings metadata: ", sum(final_data$has_earnings_metadata)))
  }
