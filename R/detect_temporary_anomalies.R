...
detect_temporary_anomalies <- function(values, lookback = 4, lookahead = 4, threshold = 3) {
  ...
  if (length(values) < min_required_length) {
    stop(paste0("Insufficient data: need at least ", min_required_length, 
                " observations for lookback=", lookback, " and lookahead=", lookahead,
                ", received: ", length(values)))
  }
  
  # Use vapply to detect anomalies for each position
  vapply(seq_along(values), detect_single_baseline_anomaly, logical(1), 
         values, lookback, lookahead, threshold)
}
