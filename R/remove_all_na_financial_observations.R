...
remove_all_na_financial_observations <- function(statements) {
  if (!is.list(statements)) {
    stop(paste0(
      "remove_all_na_financial_observations(): [statements] must be a list, not ",
      class(statements)[1]
    ))
  }
  ...
  if (length(missing_names) > 0) {
    stop(paste0(
      "remove_all_na_financial_observations(): [statements] must contain: ",
      paste(required_names, collapse = ", ")
    ))
  }

  common_metadata_cols <- c(
    "ticker",
    "fiscalDateEnding",
    "as_of_date",
    "reportedCurrency"
  ...
  cash_flow_financial_cols <- setdiff(
    names(statements$cash_flow),
    common_metadata_cols
  )
  income_statement_financial_cols <- setdiff(
    names(statements$income_statement),
    common_metadata_cols
  )
  balance_sheet_financial_cols <- setdiff(
    names(statements$balance_sheet),
    common_metadata_cols
  )

  cash_flow_cleaned <- identify_all_na_rows(
    statements$cash_flow,
    cash_flow_financial_cols,
    "cash flow"
  )
  income_statement_cleaned <- identify_all_na_rows(
    statements$income_statement,
    income_statement_financial_cols,
    "income statement"
  )
  balance_sheet_cleaned <- identify_all_na_rows(
    statements$balance_sheet,
    balance_sheet_financial_cols,
    "balance sheet"
  )
