...
#' @export
validate_continuous_quarters <- function(
  ticker_data,
  ...
  for (i in seq_along(filtered_dates)) {
    ...
    expected_quarters <- extract_quarterly_pattern(
      start_date,
      max_date,
      monthly_dates
    )
    ...
    for (j in seq_along(expected_quarters)) {
      ...
      if (actual_pos > length(filtered_dates)) {
        break
      }

      # Check if actual date matches expected date
      if (filtered_dates[actual_pos] != expected_quarters[j]) {
        break
      }
  ...
  if (!is.null(best_start_idx) && max_length > 0) {
    ...
    if (identical(cleanup_cols, default_cleanup) && row_num_col != "row_num") {
      final_cleanup_cols <- c(
        cleanup_cols[cleanup_cols != "row_num"],
        row_num_col
      )
    }
