...
#' @export
detect_time_series_anomalies <- function(values, threshold = 3, min_observations = 10) {
  
  # Input validation
  if (!is.numeric(values)) {
    stop(paste0("Argument 'values' must be numeric. Received: ", class(values)[1]))
  }
  
  if (!is.numeric(threshold) || length(threshold) != 1) {
    stop(paste0("Argument 'threshold' must be a single numeric value. Received: ",
  ...
  if (!is.numeric(min_observations) || length(min_observations) != 1) {
    stop(paste0("Argument 'min_observations' must be a single positive integer. Received: ",
                class(min_observations)[1], " of length ", length(min_observations)))
  }
  
  if (min_observations < 1) {
    stop(paste0("Argument 'min_observations' must be a single positive integer. Received: ",
                class(min_observations)[1], " with value ", min_observations))
  }
  ...
  # Check for NA values - fail condition
  if (any(is.na(values))) {
    stop(paste0("Argument 'values' contains NA values. Found ", sum(is.na(values)), 
                " NA values out of ", length(values), " total values."))
  }
  
  # Check if we have enough observations
  if (length(values) < min_observations) {
    stop(paste0("Insufficient observations for anomaly detection. Need at least ", 
                min_observations, " but got ", length(values), "."))
  }
