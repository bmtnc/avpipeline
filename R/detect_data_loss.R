...
#' @keywords internal
detect_data_loss <- function(existing_data, new_data, date_column = "fiscalDateEnding") {
  if (is.null(existing_data) || nrow(existing_data) == 0) {
    return(list(has_loss = FALSE, existing_count = 0L, new_count = nrow(new_data)))
  }
  ...
  if (!date_column %in% names(existing_data) || !date_column %in% names(new_data)) {
    return(list(has_loss = FALSE, existing_count = NA_integer_, new_count = NA_integer_))
  }
