...
calculate_per_share_metrics <- function(data, metrics_to_convert, shares_col = "commonStockSharesOutstanding") {
  # Filter to only numeric columns that exist and aren't already per-share
  existing_metrics <- intersect(metrics_to_convert, names(data))
  numeric_metrics <- existing_metrics[sapply(existing_metrics, function(col) is.numeric(data[[col]]))]
  non_per_share_metrics <- numeric_metrics[!grepl("_per_share$", numeric_metrics)]

  data %>%
    dplyr::mutate(
      valid_shares = !is.na(!!rlang::sym(shares_col)) & !!rlang::sym(shares_col) > 0,
      dplyr::across(
