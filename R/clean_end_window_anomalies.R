...
#' @export
clean_end_window_anomalies <- function(data, metric_cols, end_window_size = 5, 
                                      threshold = 3, min_observations = 10) {
  
  # Input validation for metric_cols
  if (!is.character(metric_cols) || length(metric_cols) == 0) {
    stop(paste0("Argument 'metric_cols' must be non-empty character vector, received: ",
                class(metric_cols)[1], " of length ", length(metric_cols)))
  }
  ...
  if (!is.numeric(end_window_size) || length(end_window_size) != 1 || end_window_size < 1) {
    stop(paste0("Argument 'end_window_size' must be positive integer, received: ", 
                end_window_size))
  }
  
  if (!is.numeric(threshold) || length(threshold) != 1 || threshold <= 0) {
    stop(paste0("Argument 'threshold' must be positive numeric, received: ", threshold))
  }
  
  if (!is.numeric(min_observations) || length(min_observations) != 1 || min_observations < 1) {
    stop(paste0("Argument 'min_observations' must be positive integer, received: ", 
                min_observations))
  }
  ...
  processed_list <- lapply(ticker_list, function(ticker_data) {
    ...
    for (col in metric_cols) {
      tryCatch({
        ...
        pct_changes <- c(NA, diff(values) / abs(values[-length(values)]) * 100)
        ...
        warning(paste0("Failed to clean end-window anomalies for ticker '", ticker_name, 
