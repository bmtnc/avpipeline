...
#' @keywords internal
clean_all_statement_anomalies <- function(statements, threshold = 4, lookback = 5, lookahead = 5,
                                         end_window_size = 5, end_threshold = 3, min_obs = 10) {
  if (!is.list(statements)) {
    stop(paste0("clean_all_statement_anomalies(): [statements] must be a list, not ", class(statements)[1]))
  }

  required_names <- c("cash_flow", "income_statement", "balance_sheet")
  missing_names <- setdiff(required_names, names(statements))
  if (length(missing_names) > 0) {
    stop(paste0("clean_all_statement_anomalies(): [statements] must contain: ", paste(required_names, collapse = ", ")))
  }

  if (!is.numeric(threshold) || length(threshold) != 1 || threshold <= 0) {
    stop(paste0("clean_all_statement_anomalies(): [threshold] must be a positive numeric scalar, not ", class(threshold)[1]))
  }

  message(paste0("Cleaning quarterly anomalies in financial metrics..."))
  message(paste0("Parameters: threshold=", threshold, ", lookback=", lookback, ", lookahead=", lookahead))
  message(paste0("End-window parameters: window_size=", end_window_size, ", threshold=", end_threshold, ", min_obs=", min_obs))

  metadata_cols <- c("ticker", "fiscalDateEnding", "reportedCurrency", "as_of_date")

  statement_configs <- list(
    list(name = "income_statement", display_name = "Income statement", data = statements$income_statement),
    list(name = "cash_flow", display_name = "Cash flow", data = statements$cash_flow),
    list(name = "balance_sheet", display_name = "Balance sheet", data = statements$balance_sheet)
  )
  ...
  message(paste0("- Income statement: ", nrow(cleaned_statements$income_statement), " observations"))
  message(paste0("- Cash flow: ", nrow(cleaned_statements$cash_flow), " observations"))
  message(paste0("- Balance sheet: ", nrow(cleaned_statements$balance_sheet), " observations"))
