...
#' @keywords internal
initialize_tracking_from_s3_data <- function(
  bucket_name,
  region = "us-east-1"
) {
  if (!is.character(bucket_name) || length(bucket_name) != 1) {
    stop(
      "initialize_tracking_from_s3_data(): [bucket_name] must be a character scalar"
    )
  }
  ...
  tracking_rows <- lapply(tickers, function(ticker) {
    tryCatch(
      {
        price_data <- s3_read_ticker_raw_data_single(
          ticker,
          "price",
          bucket_name,
          region
        )
        earnings_data <- s3_read_ticker_raw_data_single(
          ticker,
          "earnings",
          bucket_name,
          region
