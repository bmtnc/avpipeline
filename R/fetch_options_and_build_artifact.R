...
fetch_options_and_build_artifact <- function(
  ...
  for (ticker in tickers) {
      ...
      price_data <- tryCatch(
        {
          ...
          message(sprintf(
            "  Price data fetched (%d rows) and written to S3",
            nrow(pd)
          ))
          pd
        },
    ...
    message(sprintf(
      "  %d weekly dates derived (latest: %s)",
      length(weekly_dates),
      format(weekly_dates[1])
    ))
    ...
    if (fetch) {
      ...
      existing_dates <- if (
        !is.null(existing_options) && nrow(existing_options) > 0
      ) {
      ...
      message(sprintf(
        "  %d dates already in S3, %d to fetch",
        length(weekly_dates) - length(missing_dates),
      ...
      if (length(missing_dates) > 0) {
        ...
        if (!is.null(fetch_result)) {
          message(sprintf(
            "  Fetched: %d succeeded, %d failed, %d total rows in S3",
            fetch_result$success_count,
            fetch_result$fail_count,
    ...
    message(sprintf(
      "  Building term structure (%d observation dates)...",
    ...
    if (!is.null(ts_result) && nrow(ts_result$raw_term_structure) > 0) {
      ...
      message(sprintf(
        "  OK: %d raw rows, %d interpolated rows",
  ...
  if (nrow(combined_raw) > 0) {
    ...
    message(sprintf(
      "\nUploading artifacts to S3 (date key: %s)...",
      date_string
    ))
    ...
    upload_artifact_to_s3(
      ...
      paste0(
        "options-artifacts/",
        date_string,
        "/interpolated_term_structure.parquet"
      ),
      region
    )
    unlink(interp_path)
    message(sprintf(
      "  Interpolated term structure: %d rows",
      nrow(combined_interp)
    ))
  } else {
    message("\nNo data to upload.")
  }
  ...
  message(sprintf(
    ...
    length(tickers_succeeded),
    length(tickers_failed)
  ))
