...
clean_quarterly_metrics <- function(data, metric_cols, date_col, ticker_col,
  ...
  if (!is.character(date_col) || length(date_col) != 1) {
    stop(paste0("Argument 'date_col' must be single character string, received: ",
                class(date_col)[1], " of length ", length(date_col)))
  }
  
  if (!is.character(ticker_col) || length(ticker_col) != 1) {
    stop(paste0("Argument 'ticker_col' must be single character string, received: ",
                class(ticker_col)[1], " of length ", length(ticker_col)))
  }
  
  # Check if data is a data frame
  if (!is.data.frame(data)) {
    stop(paste0("Argument 'data' must be data frame, received: ", class(data)[1]))
  }
  
  # Handle empty data frame gracefully
  if (nrow(data) == 0) {
    return(data)
  }
  
  # Validate data frame and required columns (now that we know it's not empty)
  ...
  if (!is.numeric(lookback) || length(lookback) != 1 || lookback < 1) {
    stop(paste0("Argument 'lookback' must be positive integer, received: ", lookback))
  }
  
  if (!is.numeric(lookahead) || length(lookahead) != 1 || lookahead < 1) {
    stop(paste0("Argument 'lookahead' must be positive integer, received: ", lookahead))
  }
