...
#' @keywords internal
clean_single_statement_anomalies <- function(
  data,
  metrics,
  statement_name,
  threshold = 4,
  ...
  if (!is.character(statement_name) || length(statement_name) != 1) {
    stop(paste0(
      "clean_single_statement_anomalies(): [statement_name] must be a character scalar, not ",
      class(statement_name)[1],
      " of length ",
      length(statement_name)
    ))
  }
  if (!is.numeric(threshold) || length(threshold) != 1 || threshold <= 0) {
    stop(paste0(
      "clean_single_statement_anomalies(): [threshold] must be a positive numeric scalar, not ",
      class(threshold)[1]
    ))
  }
  if (!is.numeric(min_obs) || length(min_obs) != 1 || min_obs <= 0) {
    stop(paste0(
      "clean_single_statement_anomalies(): [min_obs] must be a positive numeric scalar, not ",
      class(min_obs)[1]
    ))
  }
