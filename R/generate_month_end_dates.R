...
generate_month_end_dates <- function(start_date, end_date) {
  ...
  if (start_date > end_date) {
    stop(paste0(
      ...
      ") must be less than or equal to 'end_date' (",
  ...
  while (current_date <= end_date) {
    ...
    next_month_first <- lubridate::ceiling_date(current_date, "month") +
      lubridate::days(1)
    current_date <- lubridate::ceiling_date(next_month_first, "month") -
      lubridate::days(1)
  }
