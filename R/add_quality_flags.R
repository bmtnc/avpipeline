...
add_quality_flags <- function(financial_statements) {
  ...
  balance_sheet_cols <- intersect(
    get_balance_sheet_metrics(),
    names(financial_statements)
  )
  cash_flow_cols <- intersect(
  ...
  financial_statements %>%
    dplyr::mutate(
      has_income_statement = dplyr::if_any(
        dplyr::all_of(income_statement_cols),
        ~ !is.na(.x)
      ),
      has_balance_sheet = dplyr::if_any(
        dplyr::all_of(balance_sheet_cols),
        ~ !is.na(.x)
      ),
      has_cash_flow = dplyr::if_any(
        dplyr::all_of(cash_flow_cols),
        ~ !is.na(.x)
      ),
      has_complete_financials = has_income_statement &
        has_balance_sheet &
        has_cash_flow,
