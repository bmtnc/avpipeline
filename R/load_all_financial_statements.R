...
load_all_financial_statements <- function(cache_paths) {
  if (!is.list(cache_paths)) {
    stop(paste0(
      "load_all_financial_statements(): [cache_paths] must be a list, not ",
      class(cache_paths)[1]
    ))
  }
  ...
  if (!all(required_names %in% names(cache_paths))) {
    stop(paste0(
      "load_all_financial_statements(): [cache_paths] must contain keys: ",
  ...
  list(
    balance_sheet = load_single_financial_type(
      cache_paths$balance_sheet,
      configs$balance_sheet
    ),
    cash_flow = load_single_financial_type(
      cache_paths$cash_flow,
      configs$cash_flow
    ),
    income_statement = load_single_financial_type(
      cache_paths$income_statement,
      configs$income_statement
    ),
    earnings = load_single_financial_type(
      cache_paths$earnings,
      configs$earnings
    )
  )
}
