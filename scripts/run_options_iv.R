...
cat(sprintf(
  ...
  if (length(result$tickers_failed) == 0) {
    "none"
  } else {
    paste(result$tickers_failed, collapse = ", ")
  }
))
...
interp <- load_options_interpolated_term_structure(
  bucket_name,
  artifact_date,
  region
)
...
if (nrow(plot_data) == 0) {
  message(sprintf(
    "No interpolated IV data for %s — skipping plot.",
    plot_ticker
  ))
} else {
  plot_data <- plot_data %>%
