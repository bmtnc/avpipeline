...
log_phase_start(
  ...
  sprintf(
    "ETF: %s | Bucket: %s | Mode: %s | Batch size: %d",
    etf_symbol,
    s3_bucket,
    fetch_mode,
    batch_size
...
log_pipeline(sprintf(
  "Total tickers: %d (%d ETF + %d additional from S3)",
  n_total,
  length(etf_tickers),
  length(setdiff(s3_tickers, etf_tickers))
))
log_pipeline(sprintf(
  "Overview present for %d tickers; %d missing (gap-fill targets)",
...
if (length(already_processed) > 0) {
  log_pipeline(sprintf(
    "Resuming from checkpoint: %d already processed, %d remaining",
    length(already_processed),
    n_tickers
  ))
}
...
log_pipeline(sprintf(
  "Fetch plan: %d to fetch | %d skipped | %d errors in planning",
...
if (n_to_fetch > 0) {
  ...
  log_pipeline(sprintf(
    "Processing %d tickers in %d batches of up to %d",
    n_to_fetch,
    n_batches,
    batch_size
  ))
        ...
        for (ticker in batch_tickers) {
          ...
          if (is.null(ticker_results)) {
            ...
            tracking <- update_tracking_after_error(
              tracking,
              ticker,
              "No responses received for ticker"
            )
